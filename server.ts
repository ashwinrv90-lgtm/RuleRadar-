...
async function startServer() {
  ...
  // Get synthetic applicants
  app.get("/api/applicants", (req, res) => {
    res.json(applicantsStore);
  });

  // Get proposals (Audit trail)
  ...
  // Generate AI Role Briefs using Gemini
...
- **PRD Ready**: Approved for sprint planning and sandbox staging deployment.`
  ...
  // Update proposal status (e.g. Approve / Reject)
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`RuleRadar server running on http://0.0.0.0:${PORT}`);
  });
}

startServer().catch(err => {
  console.error("Failed to start RuleRadar server:", err);
});
